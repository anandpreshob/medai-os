...
from monailabel.utils.prediction_cache import get_cached, store_cached
...
def send_response(datastore, result, output, background_tasks):
    ...
    if type(res_img) == str:
        if not os.path.exists(res_img):
            ...
            if not res_img.startswith("/code/predictions/"):
...
def run_inference(
    ...
    if file:
        ...
        with open(image_file, "wb") as buffer:
            ...
            request["original_filename"] = file.filename or ""
    ...
    # Check prediction cache for auto-seg models (not file uploads, not sessions, not interactive)
    INTERACTIVE_INDICATORS = ("nninter", "medsam2", "sam3")
    is_cacheable = (
        image
        and not file
        and not session_id
        and not any(k in params.lower() if params else False for k in INTERACTIVE_INDICATORS)
        and not any(k in model.lower() for k in INTERACTIVE_INDICATORS)
    )

    if is_cacheable:
        cached = get_cached(image, model)
        if cached:
            result = {"file": cached["file"], "params": cached["result_params"]}
    ...
    # Store in cache for auto-seg models
    if is_cacheable and result.get("file") and isinstance(result["file"], str):
        try:
            store_cached(image, model, result["file"], result.get("params", {}))
        except Exception:
            logger.exception("Failed to store prediction cache")
