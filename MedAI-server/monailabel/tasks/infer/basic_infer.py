...
from monailabel.tasks.infer.inference_2d import is_2d_image, run_2d_inference, reset_2d_session

# Use custom SAM2 package with medical imaging support (load_medical_slices)
from monailabel.sam2.build_sam import build_sam2_video_predictor

# Define build_sam2_video_predictor_npz as an alias for build_sam2_video_predictor
# MedSAM2 uses the same model architecture and checkpoint loading mechanism
build_sam2_video_predictor_npz = build_sam2_video_predictor

# Use custom SAM3 package with medical imaging support
from monailabel.sam3.model_builder import build_sam3_video_model
...
# Use standard tiny config since t512 is not available in official SAM2
# MedSAM2 uses the same architecture but was trained with 512 input size
medsam2_model_cfg = "configs/sam2.1/sam2.1_hiera_t.yaml"
...
class BasicInferTask(InferTask):
    ...
    def __call__(
        ...
        final_result_json = {}
        result_json = {}
        nnInter = data.get('nninter', '')
        if nnInter == "reset":
            for key, lst in self._session_used_interactions.items():
                    lst.clear()
            session.reset_interactions()
            # Also reset 2D session state
            reset_2d_session()
            logger.info("Reset nninter (both 3D and 2D sessions)")
        ...
        if image_path.endswith('.zip'):
            import zipfile
            import tempfile
            # Note: os and glob are already imported at module level
            ...
            dcm_files = glob(os.path.join(extract_dir, '**', '*.dcm'), recursive=True)
            if dcm_files:
            ...
            # For NIfTI files, set dicom_dir to None to skip DICOM processing
            if image_path.endswith('.nii.gz') or image_path.endswith('.nii'):
                dicom_dir = None
            else:
                dicom_dir = image_path.split('.nii.gz')[0]

        # Initialize DICOM metadata variables
        seriesInstanceUID = None
        instanceNumber = None
        ...
        dcm_img_sample = None
        dcm_img_sample_2 = None
        dicom_filenames = ()

        # Only process DICOM metadata if we have a valid DICOM directory
        if dicom_dir and os.path.isdir(dicom_dir):
            ...
            logger.info("NIfTI file detected, skipping DICOM metadata extraction")
        ...
        # Check for DICOM series description if available
        if dcm_img_sample is not None and 0x0008103e in dcm_img_sample.keys():
            image_series_desc = dcm_img_sample[0x0008103e].value

        # --- Load Input Image ---
        # Handle both DICOM and NIfTI inputs
        if dicom_dir and dicom_filenames:
            # DICOM: Load via ImageSeriesReader
            reader.SetFileNames(dicom_filenames)
            img = reader.Execute()
        else:
            # NIfTI: Load directly
            logger.info(f"Loading NIfTI file: {image_path}")
        ...
        # --- 2D Image Handling ---
        # Route 2D images (JPEG, PNG, etc.) to SAM3 2D processor
        if is_2d_image(image_path) and nnInter:
            logger.info(f"2D image detected, routing to SAM3 2D inference: {image_path}")
            prompts = {
                'pos_points': data.get('pos_points', []),
                'neg_points': data.get('neg_points', []),
                'pos_boxes': data.get('pos_boxes', []),
                'neg_boxes': data.get('neg_boxes', []),
            }
            return run_2d_inference(image_path, prompts, nnInter, seriesInstanceUID)
        ...
        if nnInter:
            ...
            if nnInter == "init":
                # ALWAYS initialize the session with the current image when 'init' is requested
                # This ensures we're using the freshly uploaded image, not a stale cached one
                logger.info(f"Initializing nnInteractive session. Current seriesUID: {self._session_image['seriesInstanceUID']}, New seriesUID: {seriesInstanceUID}")
                logger.info(f"Image shape: {img_np.shape}")

                # Always update the session image on init to avoid stale cache issues
                self._session_image["seriesInstanceUID"] = seriesInstanceUID if seriesInstanceUID else "unknown"
                try:
                    logger.info("Setting image and target buffer for nnInteractive session")
                    session.set_image(img_np)
                    session.set_target_buffer(torch.zeros(img_np.shape[1:], dtype=torch.uint8))
                    logger.info(f"Session initialized with image shape: {img_np.shape}")
            ...
            if len(data['pos_boxes'])!=0:
                ...
                for box in data['pos_boxes']:
                    if not self.is_prompt_used(box, "pos_boxes"):
                        ...
                        box[0]=box[0][::-1]
                        box[1]=box[1][::-1]
                        # nnInteractive expects bbox format: [[x_min, x_max], [y_min, y_max], [z, z+1]]
                        # After reversal: box[0]=[z,y_min,x_min], box[1]=[z,y_max,x_max]
                        # So: X=[box[0][2], box[1][2]], Y=[box[0][1], box[1][1]], Z=[box[0][0], box[1][0]+1]
                        bbox_coords = [[box[0][2], box[1][2]], [box[0][1], box[1][1]], [box[0][0], box[1][0] + 1]]
                        logger.info(f"Adding positive box to nnInteractive: {bbox_coords}")
                        if not _safe_interaction(lambda bc=bbox_coords: session.add_bbox_interaction(bc, include_interaction=True)):
                            return f'/code/predictions/reset.nii.gz', final_result_json
            ...
            if len(data['neg_boxes'])!=0:
                ...
                for box in data['neg_boxes']:
                    if not self.is_prompt_used(box, "neg_boxes"):
                        ...
                        # nnInteractive expects bbox format: [[x_min, x_max], [y_min, y_max], [z, z+1]]
                        bbox_coords = [[box[0][2], box[1][2]], [box[0][1], box[1][1]], [box[0][0], box[1][0] + 1]]
                        logger.info(f"Adding negative box to nnInteractive: {bbox_coords}")
                        if not _safe_interaction(lambda bc=bbox_coords: session.add_bbox_interaction(bc, include_interaction=False)):
                            return f'/code/predictions/reset.nii.gz', final_result_json
                        logger.info("Add a box")            
        ...
        #SAM2
        medsam2 = data.get('medsam2', '')
        if not nnInter and medsam2:
            if medsam2 == 'medsam2':
            ...
            # Only try to read DICOM if we have DICOM data from earlier processing
            # For NIfTI files, contrast_window and contrast_center will be None
            if contrast_window is not None and contrast_center is not None and dicom_dir and os.path.isdir(dicom_dir):
                import glob as glob_module
                image_files = glob_module.glob('{}/*'.format(dicom_dir))
                if image_files:
            ...
            if contrast_window != None and contrast_center !=None:
                ...
                with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                    inference_state = predictor.init_state(video_path=img, clip_low=contrast_center-contrast_window/2, clip_high=contrast_center+contrast_window/2, offload_video_to_cpu=True)
            else:
                logger.info(f"Calling init_state for SAM3 with offload_video_to_cpu=True")
                with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                    inference_state = predictor.init_state(video_path=img, offload_video_to_cpu=True)
                logger.info(f"init_state completed successfully. num_frames={inference_state.get('num_frames', 'unknown')}")
            ...
            # Determine instance number order for DICOM, skip for NIfTI
            instance_order_ascending = True  # Default for NIfTI
            if dicom_dir and os.path.isdir(dicom_dir):
                try:
                    reader = sitk.ImageSeriesReader()
                    dicom_filenames = reader.GetGDCMSeriesFileNames(dicom_dir)
                    if len(dicom_filenames) >= 2:
                        ...
                        instanceNumber = dcm_img_sample.get(0x00200013, None)
                        instanceNumber2 = dcm_img_sample_2.get(0x00200013, None)

                        if instanceNumber is not None and instanceNumber2 is not None:
                            instance_order_ascending = instanceNumber.value < instanceNumber2.value
                            logger.info(f"DICOM instance order: ascending={instance_order_ascending}")
                except Exception as e:
                    logger.warning(f"Could not determine DICOM instance order: {e}")
            else:
                logger.info("NIfTI file - using default frame order")

            for i in range(len(ann_frame_list)):
                if instance_order_ascending:
