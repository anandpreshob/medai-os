...
def load_video_frames(
    ...
    """
    # Check for SimpleITK Image (medical imaging)
    if isinstance(video_path, sitk.Image):
        return load_medical_slices(
    ...
    if is_bytes or is_mp4_path:
        ...
        raise NotImplementedError(
            "Only MP4 video, JPEG folder, and SimpleITK images are supported at this moment"
        )
...
def load_medical_slices(
    ...
    """
    Load medical image slices from a SimpleITK Image.
    Treats 3D volume slices as video frames for SAM processing.
    """
    import logging
    logger = logging.getLogger(__name__)
    logger.info(f"load_medical_slices called with image_size={image_size}, compute_device={compute_device}")

    img = video_path if isinstance(video_path, sitk.Image) else sitk.ReadImage(video_path)
    img_z = img.GetSize()[2]
    img_y = img.GetSize()[1]
    img_x = img.GetSize()[0]
    logger.info(f"Original image size: X={img_x}, Y={img_y}, Z={img_z}")
    ...
    logger.info(f"Resampled numpy shape: {img_npy.shape}")
    ...
    img_std = images.std()
    # Expand single channel to 3 channels (Z, 1, H, W) -> (Z, 3, H, W)
    images = torch.unsqueeze(images, 1).expand(-1, 3, -1, -1).clone()
    logger.info(f"Tensor shape: {images.shape}, memory: {images.numel() * 4 / 1024 / 1024:.2f} MB")

    video_height = img_y
    video_width = img_x

    if not offload_video_to_cpu:
        logger.info(f"Moving tensor to {compute_device}...")
        images = images.to(compute_device)
        img_mean = img_mean.to(compute_device)
        img_std = img_std.to(compute_device)
    # normalize by mean and std
    images -= img_mean
    images /= img_std
    logger.info("load_medical_slices completed successfully")
    return images, video_height, video_width
