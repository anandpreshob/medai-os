...
/**
 * MHA/MHD Image Loader using ITK-WASM
 *
 * This loader handles .mha and .mhd files commonly used in ITK/VTK pipelines.
 * MHD files are header files that reference a separate .raw data file.
 * MHA files contain both header and data in a single file.
 */
export class MhaLoader implements ImageLoader {
  name = 'mha';
  supportedExtensions = ['.mha', '.mhd'];
  ...
  async loadFromUrl(url: string): Promise<LoadedImage> {
    ...
    const filename = url.split('/').pop() || 'image.mha';
  ...
  async loadFromBuffer(buffer: ArrayBuffer, filename?: string): Promise<LoadedImage> {
    ...
    // Read the MHA/MHD file using ITK-WASM
    const { image } = await itk.readImage({
      data,
      path: filename || 'image.mha',
    });
    ...
    const imageId = `mha:${filename || Date.now()}`;
