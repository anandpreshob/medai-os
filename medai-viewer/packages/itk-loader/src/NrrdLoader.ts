...
/**
 * NRRD Image Loader using ITK-WASM
 *
 * This loader handles .nrrd files commonly used in 3D Slicer and research environments.
 */
export class NrrdLoader implements ImageLoader {
  name = 'nrrd';
  supportedExtensions = ['.nrrd', '.nhdr'];
  ...
  async loadFromUrl(url: string): Promise<LoadedImage> {
    ...
    const filename = url.split('/').pop() || 'image.nrrd';
  ...
  async loadFromBuffer(buffer: ArrayBuffer, filename?: string): Promise<LoadedImage> {
    ...
    // Read the NRRD file using ITK-WASM
    const { image } = await itk.readImage({
      data,
      path: filename || 'image.nrrd',
    });
    ...
    const imageId = `nrrd:${filename || Date.now()}`;
